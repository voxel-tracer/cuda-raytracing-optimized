...
__device__ vec3 color(const ray& r, sphere* spheres, material* materials, int numHitable, curandStatePhilox4_32_10_t * local_rand_state) {
    ray cur_ray = r;
...
__global__ void render(vec3* fb, int max_x, int max_y, int ns, const camera cam, sphere* spheres, material* materials, int numHitable, curandStatePhilox4_32_10_t * rand_state) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    if ((i >= max_x) || (j >= max_y)) return;
    int pixel_index = j * max_x + i;
